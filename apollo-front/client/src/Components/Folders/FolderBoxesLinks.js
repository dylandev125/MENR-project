...
const FolderBoxesLinks = ({ boxes }) => {
	const [finalBoxes, setFinalBoxes] = useState([]);
	const restofBoxes = boxes && boxes.slice(0, 7);
		...
		<div className='folder-extra-content'>
			<section>
				<Grid container spacing={4} justifyContent='center' alignItems='center'>
					{finalBoxes.length > 0 &&
						finalBoxes.map((bx) => (
							<Grid key={bx._id} item xs={12} sm={6} md={6} lg={4}>
								<Link
									to={`/products/${bx.econtent}`}
...
export default FolderBoxesLinks;
