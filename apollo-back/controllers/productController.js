...
const getFilteredProds = expressAsyncHandler(async (req, res) => {
  ...
  if (req.body.forFormat.length > 0) {
    filters.push({
      text: {
        query: req.body.forFormat,
        path: "characteristics.title",
      },
    });
  }
  ...
  if (req.body.forPower.length > 0) {
    filters.push({
      text: {
        query: req.body.forPower,
        path: "powerSupply.title",
      },
    });
  }
  if (req.body.forCont.length > 0) {
    filters.push({
      text: {
        query: req.body.forCont,
        path: "controller.title",
      },
    });
  }
  if (req.body.forSize.length > 0) {
    filters.push({
      text: {
        query: req.body.forSize,
        path: "diagonale.title",
      },
    });
  }
